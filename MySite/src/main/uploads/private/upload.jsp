...
<%
	...
	boolean isSuccess=UploadDao.getInstance().insert(dto);
	//6. 응답한다. 
...
<head>
<meta charset="UTF-8">
<title>/uploads/private/upload.jsp</title>
</head>
<body>
	<%if(isSuccess){ %>
		<p>
			<%=writer %> 님이 업로드한 <%=orgFileName %> 파일을 저장했습니다.
			<a href="${pageContext.request.contextPath }/uploads/list.jsp">목록보기</a>
		</p>
	<%}else{ %>
		<p>
			업로드 실패!
